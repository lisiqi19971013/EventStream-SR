...
from utils.ncifarDatasetClassification import NCifar
...
trainDataset = NCifar(basicPath="../dataset/Cifar10-DVS/SR_Train/HR/", train=True)
if mode == "HrGt":
    testDataset = NCifar(basicPath="../dataset/Cifar10-DVS/SR_Test/HR/", train=False)
elif mode == "previousWork":
    testDataset = NCifar(basicPath="../dataset/Cifar10-DVS/SR_Test/previousWork/", train=False)
elif mode == "HrPre":
    testDataset = NCifar(basicPath="../dataset/Cifar10-DVS/Res_Conv/HRPre/", train=False)
elif mode == "Lr":
    trainDataset = NCifar(basicPath="../dataset/Cifar10-DVS/SR_Train/LR/", train=True)
    testDataset = NCifar(basicPath="../dataset/Cifar10-DVS/SR_Test/LR/", train=False)
else:
    raise ValueError

bs = 16
trainLoader = Loader(dataset=trainDataset, device="cuda", batch_size=bs, num_workers=8, pin_memory=True)
testLoader = Loader(dataset=testDataset, device="cuda", batch_size=bs, num_workers=8, pin_memory=True)

shape = (9, 128, 128) if mode != "Lr" else (9, 64, 64)
model = Classifier(voxel_dimension=shape, num_classes=10).to("cuda")
...
model, epoch0 = checkpoint_restore(model, savePath, name="ckptBest")
...
for epoch in range(epoch0+1, maxEpoch):
    ...
    checkpoint_save(model=model, path=savePath, epoch=epoch, name="ckpt", device='cuda')
